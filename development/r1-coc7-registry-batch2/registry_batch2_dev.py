...
BATCH1_DIR = ROOT / "development" / "r1-coc7-registry-batch1"
if str(BATCH1_DIR) not in sys.path:
    sys.path.insert(0, str(BATCH1_DIR))

import registry_batch1_dev as batch1  # noqa: E402

BATCH = json.loads((HERE / "occupations_batch2.json").read_text(encoding="utf-8"))
REGISTRY_ID = "COC7_RECOVERY_REGISTRY_R1_BATCH2_DEV_V1"
PARENT_REGISTRY_ID = batch1.REGISTRY_ID
FROZEN_ANCESTOR_REGISTRY_ID = batch1.PARENT_REGISTRY_ID
...
INTERPERSONAL = batch1.INTERPERSONAL
CLASSIC_ERAS = batch1.CLASSIC_ERAS
...
def resolve_skill(
    ...
    *,
    ...
    if record is None:
        return batch1.resolve_skill(key, dex=dex, edu=edu, era=era)
    if record.get("era_scope") == "MODERN" and _era_key(era) in CLASSIC_ERAS:
...
def resolve_occupation(
    ...
    *,
    ...
    if record is None:
        parent_result = batch1.resolve_occupation(
            key,
    ...
    if record.get("era_scope") == "MODERN" and _era_key(era) in CLASSIC_ERAS:
        return {"status": "BLOCKED", "code": "OCCUPATION_NOT_AVAILABLE_IN_ERA", "occupation_id": key, "era": era}
...
def _materialized_skill_ids() -> set[str]:
    return set(batch1.parent.SKILLS) | set(batch1.SKILL_EXTENSIONS) | set(SKILL_EXTENSIONS)
...
def validate_record_references(occupation_id: str) -> dict:
    ...
    record = OCCUPATIONS.get(key)
    if record is None:
        return batch1.validate_record_references(key)
    if occupation_slot_count(record) != 8:
        return {"status": "BLOCKED", "code": "OCCUPATION_SKILL_SLOT_COUNT_INVALID", "occupation_id": key}
    unresolved = []
...
def batch1_compatibility() -> dict:
    checks = []
    for occupation_id in batch1.OCCUPATIONS:
        direct = batch1.resolve_occupation(occupation_id)
...
def registry_summary() -> dict:
    return {
        ...
        "frozen_ancestor_registry_id": FROZEN_ANCESTOR_REGISTRY_ID,
        "source_sha256": SOURCE_SHA256,
        "batch1_occupations": len(batch1.OCCUPATIONS),
        "batch2_occupations": len(OCCUPATIONS),
        "cumulative_occupations": len(batch1.OCCUPATIONS) + len(OCCUPATIONS),
        "batch2_skill_extensions": len(SKILL_EXTENSIONS),
