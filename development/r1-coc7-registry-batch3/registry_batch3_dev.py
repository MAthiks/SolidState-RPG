...
BATCH2_DIR = ROOT / "development" / "r1-coc7-registry-batch2"
if str(BATCH2_DIR) not in sys.path:
    sys.path.insert(0, str(BATCH2_DIR))

import registry_batch2_dev as batch2  # noqa: E402

BATCH = json.loads((HERE / "occupations_batch3.json").read_text(encoding="utf-8"))
REGISTRY_ID = "COC7_RECOVERY_REGISTRY_R1_BATCH3_DEV_V1"
PARENT_REGISTRY_ID = batch2.REGISTRY_ID
FROZEN_ANCESTOR_REGISTRY_ID = batch2.FROZEN_ANCESTOR_REGISTRY_ID
...
INTERPERSONAL = batch2.INTERPERSONAL
CLASSIC_ERAS = batch2.CLASSIC_ERAS
MODERN_ERAS = {"MODERN", "MODERN_DAY", "CONTEMPORARY"}
...
def resolve_skill(
    ...
    *,
    ...
    if record is None:
        return batch2.resolve_skill(key, dex=dex, edu=edu, era=era)
...
def resolve_occupation(
    ...
    *,
    ...
    if record is None:
        parent_result = batch2.resolve_occupation(
            key,
...
def _materialized_skill_ids() -> set[str]:
    return (
        set(batch2.batch1.parent.SKILLS)
        | set(batch2.batch1.SKILL_EXTENSIONS)
        | set(batch2.SKILL_EXTENSIONS)
        | set(SKILL_EXTENSIONS)
    )


def _family_is_materialized(family: str) -> bool:
    prefix = str(family).upper() + "_"
    return any(skill_id.startswith(prefix) for skill_id in _materialized_skill_ids())
...
def _iter_reference_requirements(slot: dict):
    ...
    for family in slot.get("choice_one_of_families", []):
        yield ("FAMILY", family)
...
def validate_record_references(occupation_id: str) -> dict:
    ...
    record = OCCUPATIONS.get(key)
    if record is None:
        return batch2.validate_record_references(key)
    if occupation_slot_count(record) != 8:
        return {"status": "BLOCKED", "code": "OCCUPATION_SKILL_SLOT_COUNT_INVALID", "occupation_id": key}
    unresolved = []
    for slot in record["skill_slots"]:
        for kind, value in _iter_reference_requirements(slot):
            if kind == "FAMILY":
                ...
                # Some choices are intentionally classic-safe alternatives to a modern skill.
                # Reference validation checks materialization, not whether every option is valid in every era.
...
def _parent_occupation_ids() -> set[str]:
    return set(batch2.OCCUPATIONS) | set(batch2.batch1.OCCUPATIONS)


def batch2_compatibility() -> dict:
    checks = []
    for occupation_id in sorted(_parent_occupation_ids()):
        direct = batch2.resolve_occupation(occupation_id)
...
def registry_summary() -> dict:
    parent_count = batch2.registry_summary()["cumulative_occupations"]
    return {
        "registry_id": REGISTRY_ID,
        "parent_registry_id": PARENT_REGISTRY_ID,
        "frozen_ancestor_registry_id": FROZEN_ANCESTOR_REGISTRY_ID,
        ...
        "batch3_occupations": len(OCCUPATIONS),
        "cumulative_occupations": parent_count + len(OCCUPATIONS),
        "batch3_skill_extensions": len(SKILL_EXTENSIONS),
