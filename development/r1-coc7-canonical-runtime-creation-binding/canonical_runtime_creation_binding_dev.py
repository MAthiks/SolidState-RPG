...
from integrated_adjudication_r1_c4 import SourceBackedRuntimeR1C4, INTEGRATION_ID as C4_INTEGRATION_ID  # noqa: E402
from scenario_router_r1_c4 import ROUTES, ROUTER_ID  # noqa: E402
from registry_r1_c4 import REGISTRY_ID  # noqa: E402
from source_adapter_r1 import SOURCE_SPECS  # noqa: E402
from runtime_r1.core import CHECKPOINT_FLOOR, sha  # noqa: E402

MODULE_ID = 'COC7_CANONICAL_RUNTIME_CREATION_BINDING_R1_DEV_V1'
PARENT_CREATION_MODULE_ID = creation4.MODULE_ID
PARENT_HARDENED_PROOF = 2714
FROZEN_RUNTIME_INTEGRATION_ID = C4_INTEGRATION_ID
...
def canonical_binding_status(*, runtime) -> dict:
    if not isinstance(runtime, SourceBackedRuntimeR1C4):
        return _blocked('C4_SOURCE_BACKED_RUNTIME_REQUIRED')
    try:
    ...
    if not isinstance(scenario, dict):
        return _blocked('C4_SCENARIO_BINDING_REQUIRED')
    if set(scenario) != EXPECTED_SCENARIO_RUNTIME_KEYS:
        return _blocked('C4_SCENARIO_BINDING_SHAPE_INVALID')

    key = str(scenario.get('scenario_key', '')).upper()
    route = ROUTES.get(key)
    if route is None:
        return _blocked('SCENARIO_NOT_REGISTERED', scenario_key=key)
    if route.canonical_path_ready is not True or route.release_class != 'PASS_REAL':
        return _blocked('C4_SCENARIO_ROUTE_NOT_PLAY_READY', scenario_key=key, release_class=route.release_class)

    expected_hashes = {}
    for source_id in route.source_ids:
        spec = SOURCE_SPECS.get(source_id)
        if spec is None:
            return _blocked('C4_EXPECTED_SOURCE_IDENTITY_MISSING', source_id=source_id)
    ...
    if scenario != expected:
        ...
        return _blocked('C4_SCENARIO_BINDING_MISMATCH', scenario_key=key, mismatches=mismatches)
...
def commit_canonical_investigator(*, runtime, player_id: str, ready: dict) -> dict:
    ...
    binding = canonical_binding_status(runtime=runtime)
    if binding.get('status') != 'READY':
        return _fail_closed(runtime, before, binding.get('code', 'C4_SCENARIO_BINDING_INVALID'), binding=binding)
    state = runtime._get_state()
