...
for sk in SC:
 for n in (1,2,3,4):
  ...
  o=rt.new_session(sk,players(n),require_sources=True); ck(tag+'_session',o.get('status')=='SESSION_READY',o)
...
def save_resume_case(sk,n,cuts):
 ...
 ck(tag+'_session',r.new_session(sk,players(n),True).get('status')=='SESSION_READY'); st,_=r.db.state(); cmap=copy.deepcopy(st['interface_session']['control_map']); r.close()
 ...
 for cut in cuts:
  ...
  sid=f'{tag}_{cut}'; sv=rr.save(sid); ck(f'{tag}_{cut}_save',sv.get('status')=='SAVED',sv); rs=rr.restore_file(f'{sid}.json',True); ck(f'{tag}_{cut}_restore',rs.get('status')=='RESTORED_STRICT',rs)
...
rt=OfflinePlayableRuntimeV1(ROOT,Path(tempfile.mkdtemp())/'tamper.sqlite'); ck('T_session',rt.new_session('scenario3',players(4),True).get('status')=='SESSION_READY'); st,_=rt.db.state(); cmap=st['interface_session']['control_map']
...
def reject(name,mut,rea=False):
 b=copy.deepcopy(base);mut(b);reauth(b) if rea else None;fn=f'ss333_{name}.json';(ROOT/'saves'/fn).write_text(json.dumps(b));before=minimal(rt);res=rt.restore_file(fn,True);after=minimal(rt);ck('T_'+name+'_closed',res.get('status')=='FAIL_CLOSED',res);ck('T_'+name+'_unchanged',before==after);(ROOT/'saves'/fn).unlink(missing_ok=True)
...
report={'checkpoint':333,'id':'MULTIPLAYER_FULL_STACK_RELEASE_AUDIT_V2','result':'PASS','parent':332,'source_backed_recertification_executed':True,'core_matrix':'5 scenarios x 1-4 players','core_events_per_case':12,'save_resume_multi_cuts':{'scenario3_1_to_4_players':[1,6,11],'scenario4_to_7_4players':[6]},'keeper_to_player_leaks':leaks,'passed':passed,'total':len(checks)}
