...
from .multi_scenario_status_resolver import MultiScenarioStatusResolver
from .playloop import ChoiceMenu

DEFAULT_SCENARIO_REGISTRY = {
    "scenario3": {"title": "Les Maudits"},
    "scenario4": {"title": "Les Registres de Brume"},
    "scenario5": {"title": "L'Antre de l'abomination"},
    "scenario6": {"title": "Muse équivoque aux yeux de sel gemme"},
    "scenario7": {"title": "L'Explorateur assassiné"},
}

class ScenarioSelectionInterfaceV1:
    """Player-safe scenario selection surface backed by certified status records."""
    def __init__(self, scenario_root, registry=None):
        self.scenario_root = scenario_root
        self.registry = deepcopy(registry or DEFAULT_SCENARIO_REGISTRY)

    def _resolve(self, scenario_key):
        if scenario_key not in self.registry:
            return None
        return MultiScenarioStatusResolver.resolve(self.scenario_root, scenario_key)

    def list_scenarios(self):
        rows = []
        for key, meta in self.registry.items():
            status = self._resolve(key)
            rows.append({
                "scenario_key": key,
                "title": meta["title"],
                "status": status["status"],
                "selectable": bool(status.get("pass_real")),
            })
        return {"status": "READY", "code": "SCENARIO_REGISTRY_VIEW", "scenarios": rows}

    def select(self, scenario_key):
        resolved = self._resolve(scenario_key)
        if resolved is None:
            return {"status": "BLOCKED", "code": "SCENARIO_NOT_IN_REGISTRY"}
        public = {
            "scenario_key": scenario_key,
            "title": self.registry[scenario_key]["title"],
            "certification_status": resolved["status"],
        }
        if not resolved.get("pass_real"):
            return {"status": "BLOCKED", "code": "SCENARIO_NOT_CERTIFIED", "selection": public}
        return {"status": "SELECTED", "code": "SCENARIO_CERTIFIED", "selection": public}
...
class PlayerInterfaceV1:
    ...
    def _inventory(self, character_id):
        rows = self.e.db.conn.execute(
            """SELECT i.object_id,i.quantity,m.object_type
               ...
               ORDER BY i.object_id""", (character_id,)
        ).fetchall()
        return [
    ...
    def decision_prompt(self, player_id, character_id, mode="NORMAL_LIBRE", options=None):
        ...
        if mode == "NORMAL_LIBRE":
            return {
                "status": "DECISION_READY", "code": "OPEN_PROMPT_ONLY",
                "prompt": "Que fais-tu ?", "menu": None, "status_panel": panel,
            }
        ...
        if menu["status"] != "RESOLVED":
            return menu
        return {
            "status": "DECISION_READY", "code": "ASSISTED_THREE_PLUS_FREE",
            "prompt": "Que fais-tu ?", "menu": menu, "status_panel": panel,
        }
...
class LaunchChainV1:
    ...
    def prepare_session(self, scenario_key, player_ids):
        ...
        return {
            "status": "SESSION_READY", "code": "LAUNCH_CHAIN_V1_READY",
            "commit": tx["new_commit"], "session": session_record, "trace": trace,
        }
