...
# Scenario 4: only targets that are also explicit nodes in the source 'Mène vers' table.
brume=[
...
# Scenario 5: preserve previously proven exact transition, now through generic gate.
...
# Scenario 6: exact source-language section handoff when Justine is obtained/found.
...
# Scenario 7: structural headings and clue anchors alone are not causal transitions.
...
# Keeper firewall: no source text is present in player-safe provenance.
...
# Historical status files unchanged / no promotion.
...
# Regression: checkpoint 316 and 315 suites still pass.
